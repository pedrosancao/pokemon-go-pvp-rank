import levelConstants, { indexToLevel, levelToIndex, } from './level-constants.js';
...
export function calculateProducts({ pokedexEntry, attackStat, defenseStat, healthStat, levelConstant, }) {
...
export function buildRank({ pokedexEntry, maxCP, maxLevel = 40, minimumStatValue: minStat = 0, }) {
    const rankEntries = [];
                ...
                for (let levelIndex = initialIndex; levelIndex >= 0; levelIndex -= 1) {
                    ...
                    if (products.cp <= maxCP) {
                        rankEntries.push(Object.assign({ attackStat,
                            defenseStat,
                            healthStat, level: indexToLevel({ index: levelIndex }) }, products));
                        break;
                    }
                }
            }
...
export default function calculateRank({ pokedexEntry, refAttackStat, refDefenseStat, refHealthStat, maxCP, maxLevel = 40, minimumStatValue = 0, }) {
